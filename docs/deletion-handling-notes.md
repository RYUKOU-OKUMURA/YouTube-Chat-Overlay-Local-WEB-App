...
2. **修正済み: 撤回プレースホルダが別 ID で届くケースを元コメントへ紐づける**
   ...
   撤回プレースホルダは list の履歴ウィンドウにのみ現れるため(下記3参照)、30秒間隔の list reconcile から同じ著者・時刻アンカーの解決を通して適用する。
...
3. **修正済み(方針変更): list reconcile を60秒間隔で復活する**
   ...
   このため撤回・削除の検知用に `listLiveChatDeletionEvents`(`liveChatMessages.list` 1回/30秒)を復活し、プレースホルダ・tombstone・deletion イベントを抽出して既存の著者アンカー解決へ流し込む(`scheduleDeletionReconcile`)。`maxResults: 2000`(API 上限)で履歴ウィンドウを深く取り、投稿から時間が経って削除されたコメントの tombstone も拾う。3時間配信で約360回の list 呼び出し(1回5ユニット≒1800ユニット)であり、日次 quota 内に収まる。
...
- **list reconcile**: 撤回検知のため `liveChatMessages.list` を**30秒間隔で1回**呼ぶ(配信中のみ、`maxResults: 2000`・`hl: "ja"`)。初回全ページ走査や `pollingIntervalMillis` 間隔での高頻度ポーリングは行わない。削除・撤回の表示反映は最大30秒遅延する。
