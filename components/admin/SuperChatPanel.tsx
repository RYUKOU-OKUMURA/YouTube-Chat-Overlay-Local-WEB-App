              ...
              return (
                <div
                  ...
                  className={`grid gap-2 rounded-xl border px-3 py-3 ${
                    deleted ? "border-slate-300 bg-slate-100" : active ? "border-red-500 bg-red-50" : "border-amber-200 bg-amber-50"
                  }`}
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-bold text-white ${deleted ? "bg-slate-400" : "bg-amber-500"}`}>
                      {message.amountText ?? paidEventLabel}
                    </span>
                    <Badge tone="amber" className="border-0 bg-amber-100">{paidEventLabel}</Badge>
                    {deletedLabel ? <Badge tone="slate" className="border-0 bg-slate-200 text-slate-700">{deletedLabel}</Badge> : null}
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      ...
                      disabled={deleted || busyAction === `show-${message.id}`}
                    >
                      表示
                    </Button>
