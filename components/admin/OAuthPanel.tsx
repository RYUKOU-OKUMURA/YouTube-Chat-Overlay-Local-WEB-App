...
export function OAuthPanel({
  ...
  return (
    <Panel
      ...
      actions={<Badge tone={connected ? "green" : "amber"}>{connected ? "認可済み" : "未認可"}</Badge>}
    >
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2 text-sm text-slate-700">
          {connected ? <ShieldCheck className="h-4 w-4 text-emerald-600" /> : <ShieldAlert className="h-4 w-4 text-amber-600" />}
