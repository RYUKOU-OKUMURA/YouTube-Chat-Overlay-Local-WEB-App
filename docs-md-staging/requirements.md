...
1. 配信URLを空欄にして「自動検出して開始」を押す。手動指定したい場合は管理画面に YouTube Live URL を入力する。
2. アプリが現在配信中のライブを自動検出する、または入力URLから videoId を抽出する。
3. アプリが YouTube API で liveChatId を取得する。
...
- 通常ユーザーが操作しない表示専用ページ
- YouTube ライブ映像プレビューや管理 UI は表示しない
...
- overlay URL は固定の `/overlay` とする。
- 互換用の `/overlay/{overlayToken}` では overlay token が一致しない Socket.IO 購読を切断する。
