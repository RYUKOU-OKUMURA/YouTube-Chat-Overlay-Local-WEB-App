...
2. **修正済み: 撤回プレースホルダが別 ID で届くケースを元コメントへ紐づける**
   ...
   quota 消費を抑えるため、`liveChatMessages.list` による補完は停止した。撤回プレースホルダは stream で届いた範囲だけ同じ著者・時刻アンカーの解決を通して適用する。

3. **修正済み: list API 補完を停止する**
   削除・撤回の補完で使っていた `youtube.liveChatMessages.list` は quota 消費増幅の原因になるため停止した。
   コメント取得と stream 由来の削除イベント処理は `youtube.youtube.v3.liveChat.messages.*` の stream に一本化する。
...
- **list 補完**: quota 消費を抑えるため停止。初回全ページ走査や `pollingIntervalMillis` 間隔の `liveChatMessages.list` は行わない。
