# YouTube Chat Overlay Local WEB App

YouTube Live のコメントを取得し、配信者が選んだコメントだけを OBS Browser Source に表示するローカル専用 Web アプリです。

管理画面でコメントと YouTube ライブ映像プレビューを確認し、表示したいコメントをクリックまたは「表示」ボタンで OBS オーバーレイに出します。表示したコメントは自動では消えず、次のコメント表示または「非表示」まで残ります。OBS オーバーレイは透明背景のコメント表示専用です。

## Local Only

このアプリは個人のローカル PC で使う前提です。管理画面、API、Socket.IO にログインや管理トークンはありません。

- `localhost:3000` で起動して使ってください。
- LAN、VPN、トンネル、公開サーバーには公開しないでください。
- OBS Browser Source には同じ PC から固定 URL `http://localhost:3000/overlay` を指定してください。
- `data/youtube-token.json` には YouTube OAuth トークンが平文で保存されます。

## Features

- YouTube OAuth 接続
- YouTube Live URL から videoId を抽出
- 接続済みチャンネルの現在配信中ライブを自動検出
- YouTube Data API で liveChatId と配信タイトルを取得（URL指定時はチャンネル名も取得）
- `liveChat.messages.stream` によるライブチャット取得
- 再接続上限付きの stream lifecycle 管理
- コメント一覧のリアルタイム更新
- コメント検索、最新追従、未表示/重要フィルタ、表示済みラベル
- 削除・取消コメントの「削除済み」表示と OBS 再表示防止
- 通常 Unicode 絵文字を含むコメント本文の保持
- コメントの手動表示、非表示、本文コピー
- 操作画面タブでの YouTube ライブ映像プレビュー
- 3分間隔の同時視聴者数表示と手動更新
- 管理画面専用の Super Chat 履歴枠
- OBS 用固定 overlay URL のコピー
- 透明背景・コメント表示専用の OBS オーバーレイ
- 通常コメントカードと Super Chat 専用カード
- 吹き出しのしっぽが本文を覆わない Overlay レイアウト
- テストコメント、テストスパチャ
- テーマプリセットと表示調整
- Socket.IO による管理画面、オーバーレイ間の同期

## Setup

```bash
npm install
cp .env.example .env.local
```

`.env.local` に Google OAuth の値を設定します。

```bash
GOOGLE_CLIENT_ID=your-google-client-id
GOOGLE_CLIENT_SECRET=your-google-client-secret
GOOGLE_REDIRECT_URI=http://localhost:3000/api/youtube/callback
PORT=3000
```

Google Cloud Console では YouTube Data API v3 を有効化し、OAuth クライアントの承認済みリダイレクト URI に `http://localhost:3000/api/youtube/callback` を登録してください。

## Run

開発起動:

```bash
npm run dev
```

本番ビルド:

```bash
npm run build
npm start
```

管理画面:

```text
http://localhost:3000/admin
```

`/` は `/admin` へリダイレクトされます。

## OBS Browser Source

管理画面の「OBS URLをコピー」から overlay URL を取得し、OBS の Browser Source に設定します。

- URL: `http://localhost:3000/overlay`
- Width: `1920`
- Height: `1080`
- Custom CSS: 原則不要
- 背景: アプリ側で透明化

YouTube ライブ映像プレビューは管理画面の操作画面だけに表示します。OBS オーバーレイには映像プレビューや管理 UI を出さず、選択したコメントカードだけを透明背景で表示します。

1280x720 などの小さめのキャンバスにも対応するため、オーバーレイ側では表示領域に応じた compact レイアウトを使います。

## Usage

1. 管理画面を開く。
2. YouTube OAuth に接続する。
3. OBS Browser Source に overlay URL を設定する。
4. 配信URLを空欄のまま「自動検出して開始」を押す。手動指定したい場合だけ YouTube Live URL を入力して開始する。
5. 必要なら「テストコメント」または「テストスパチャ」で表示確認する。
6. 操作画面の YouTube ライブ映像プレビューとコメント一覧を確認する。
7. コメント一覧で表示したいコメントをクリック、または「表示」を押す。
8. 消したい場合は「非表示」を押す。
9. 配信後は「停止」を押す。

## Current Behavior

- コメント取得はポーリングではなく stream 方式です。
- 配信URLが空欄の開始操作では `liveBroadcasts.list` で現在配信中の自分のライブを1回だけ検出します。`search.list` は使いません。
- コメント履歴は保存しません。アプリ起動中のメモリに最大300件だけ保持します。
- Super Chat 履歴は管理画面用にアプリ起動中のメモリへ最大100件保持します。
- 削除・取消コメントは一覧から消さず、本文を「このコメントは削除されました。」または「このコメントは投稿者により取り消されました。」へ置き換えます。単一コメントを表示する直前に最大200件の最新一覧で確認し、削除済み・確認できない候補は表示しません。
- 削除確認は単一コメント表示の操作時だけに行います。すでに表示中のコメントが後から削除されても、次の表示操作または手動非表示までは残ります。
- 通常コメント本文は `textMessageDetails.messageText` を優先し、Super Chat は `superChatDetails.userComment` も本文候補に含めます。どちらも空の場合は `displayMessage` に fallback します。
- YouTube独自絵文字の画像化や shortcode 変換は未実装です。
- 同時視聴者数はコメント取得中に3分間隔で更新します。手動更新もサーバー側で同じ3分間隔に抑制し、連打でYouTube APIを消費しないようにしています。視聴者数非表示や取得不可の場合は件数を出しません。
- 300件を超える場合は、未表示 Super Chat、未表示の配信者/モデレーター/メンバー、未表示通常、表示済み重要、表示済み通常の順で優先保持します。
- 同一 `platformMessageId` は重複表示しません。
- 表示したコメントは自動消去されません。
- 同じ videoId の開始操作はサーバー側で冪等化し、開始処理は直列化します。
- stream 再接続は最大8回、初期2秒、最大60秒のバックオフで行います。YouTube の top-level JSON array stream を batch ごとに分割し、JSON応答の途中終了は一時的な stream 切断として自動再接続します。5秒未満の短時間 close が続く場合は、batch 受信の有無にかかわらずエラーとして停止します。
- YouTube 接続解除時は、既存のコメント取得 stream も停止します。
- ライブ未開始、終了済み、チャット無効、認可不足、parser/応答形式エラー、通信エラーは区別して表示します。
- 管理画面は初回接続と明示同期で full state を受け取り、通常のコメント流入中は `comment:new` と `broadcast:status` の差分で更新します。OBS overlay は overlay state のみを Socket.IO で同期します。
- 壊れた JSON や schema 不一致の API request body は `VALIDATION_ERROR` として整形します。
- pin/unpin、表示秒数、複数モデレーター、ユーザー管理はありません。
- YouTube 接続状態は token/env の確認が中心で、軽量な実 API 疎通確認は行っていません。

## Local Data

Git 管理しないローカルファイル:

- `data/settings.json`
  - `theme`
  - `lastBroadcastUrl`
- `data/youtube-token.json`
  - `accessToken`
  - `refreshToken`
  - `expiryDate`

`data/settings.json` が存在しない、または壊れている場合は、起動時にデフォルト設定を生成します。

## Verification

```bash
npm test
npx tsc --noEmit
npm run lint
npm run build
npm run test:e2e
```

macOS の sandbox や CI 環境によっては Playwright Chromium が起動できない、またはアニメーション待ちで E2E がフレークする場合があります。その場合は失敗内容を記録し、unit/type/build の結果と合わせて判断してください。

## Known Limitations

- 管理画面、API、Socket.IO に認証はありません。ローカル専用です。
- OAuth `state` 検証は未実装です。
- YouTube独自絵文字の画像表示には未対応です。

## Documents

- 削除・取消コメント処理の設計メモ: `docs/deletion-handling-notes.md`
